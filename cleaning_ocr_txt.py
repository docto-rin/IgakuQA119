...
def split_english_text(text: str) -> tuple[str, list[str]]:
    ...
    # 選択肢のパターンで分割
    choice_matches = re.finditer(r'([abcdeABCDE])[.\s]([^abcdeABCDE]*?)(?=[abcdeABCDE][.\s]|$)', choices_text, re.IGNORECASE)
...
def split_choices_by_sequence(choices_text: str, question_num: str) -> list[str]:
    ...
    for letter in choice_letters:
        ...
        if match:
            ...
            logging.info(f"問題 {question_num}: 選択肢 {letter} が位置 {match.start()} で見つかりました")
...
def split_questions(text: str, input_file: str, start: int = 1, end: int = 50) -> str:
    ...
    # まず連問のパターンを探す
    shared_text_pattern = r'(次の文を読み、\d+、\d+の問いに答えよ。)(.*?)(?=\d+[A-Z]\d+|$)'
    for match in re.finditer(shared_text_pattern, clean_text, re.DOTALL):
        ...
        if len(nums) >= 2:
            ...
            logging.info(f"連問を検出: {q1_full}と{q2_full}が共有テキスト「{content[:50]}...」を持ちます")
